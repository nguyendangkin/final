...
export class NotificationsService {
  ...
  async markAllAnnouncementsAsRead(userId: string) {
    ...
    const unreads = await this.announcementRepo
      .createQueryBuilder('announcement')
      .leftJoin(
    ...
    if (unreads.length > 0) {
      const newReads = unreads.map((a) =>
        this.readRepo.create({ userId, announcementId: a.id }),
      );
  ...
  async getUnreadCounts(userId: string) {
    ...
    let systemCount = 0;
    if (user) {
      systemCount = await this.announcementRepo
        .createQueryBuilder('announcement')
        .leftJoin(
          'user_announcement_read',
          'read',
