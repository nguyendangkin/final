...
export class FavoritesController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Get()
    async getFavorites(@Request() req, @Query('page') page: number = 1, @Query('limit') limit: number = 10) {
        return this.favoritesService.getFavorites(req.user, Number(page), Number(limit));
    }

    @UseGuards(AuthGuard('jwt'))
    @Get('check/:carId')
    async checkIsFavorited(@Request() req, @Param('carId') carId: string) {
        const isFavorited = await this.favoritesService.checkIsFavorited(req.user, carId);
        return { isFavorited };
    }
}
